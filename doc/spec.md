...
1. **結構化項目**：Planner API `GET /api/v1/planner/items?start_date=<今天-14天>`，只保留上述 4 門課，且 `plannable_type` 為 `assignment` / `quiz` / `discussion_topic`（排除公告）。**不使用**繳交狀態：網頁公開且供同學共用，完成與否由每位使用者自行勾選。
2. **非結構化項目**：預習、只寫在頁面中的作業與考試。來源為各課 Modules 中的 Page 內文，以及每門課的「Module 大綱」虛擬頁面（所有 Module 名稱，用來抓只寫在 Module 名稱的考試）。
   - Canvas 網址的 `verifier` 參數會被移除（它可讓人免登入下載檔案，而網頁是公開的），連結需登入 Canvas 才能開啟。
...
3. **Canvas 作業說明**：對每個結構化項目呼叫 Assignment / Quiz / Discussion API 取得說明，交給 page-extractor 產生一句話摘要（以 `updated_at` 快取；Quiz API 無 `updated_at`，改用說明文字雜湊）。快取中沒有的項目一律判讀。
...
`id`、`course`、`title`、`type`（assignment / quiz / discussion_topic / prep / exam / task）、`due`（ISO，可為 null）、`url`（資源本身的連結，可為 null）、`summary`（一句英文描述）、`source_url` / `source_title`（僅頁面項目：出處課程頁面）、`note`（`optional` / `time TBD`）、`source`（canvas / page）、`first_seen`。
